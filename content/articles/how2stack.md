...
ret2test(system,'/bin/sh')->ret2syscall->ret2libc

# 0.0 Return 2 libc

ret2libc即控制函数的执行 libc中的函数，通常是返回至某个函数的plt处或者函数的具体位置(即函数对应的got表项的内容)。一般情况下，我们会选择执行system("/bin/sh")，故而此时我们需要知道system函数的地址。

这里我们需要注意函数调用栈的结构，如果ret返回到的plt中的地址，那么相当于正常调用函数，这是栈顶应该是一个返回地址，其后才是对应的参数内容。

当需要自己构造字符串时，可以使用`gets`函数，这里仍然需要注意上述的栈结构，get地址下方是另一个ret地址，随后才是第一个参数，因此ret地址需要指向一个pop。例如
...
当原二进制中没有system时，则需要泄露libc基址，此时可以使用`puts`函数泄露相关信息，一般会利用思路如下，这里的技巧是二次触发漏洞。
...
#  Stack smash / Using abort message
...
当ELF文件采用动态链接时，got表会采用延迟绑定技术。当第一次调用某个libc函数时，程序会调用_dl_runtime_resolve函数对其地址解析。因此，我们可以利用栈溢出构造ROP